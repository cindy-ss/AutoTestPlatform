...
exports = module.exports = internals.Route = function (options, connection, plugin) {
    ...
    Hoek.assert(options.handler || (options.config && options.config.handler), 'Missing or undefined socketHandler:', options.method, options.path);
    ...
    // Apply settings in order: {connection} <- {socketHandler} <- {realm} <- {route}
...
internals.Route.prototype.rebuild = function (event) {
    ...
    if (this._extensions.onPostHandler.nodes) {
        cycle.push(this._extensions.onPostHandler);                 // An error from here on will override any result set in socketHandler()
    }
